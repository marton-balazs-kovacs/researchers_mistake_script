...
We received `r filter(distrib_channel_count, distribution_channel == "anonymous") %>% pull(n)` answers from the redistribution of our survey. We decided to keep these answers in for the creation of the groups but exclude them from the analysis.
...
n_response_left_after_excluding_ambiguous(processed_mistake_type_coding, mistake_type)

n_response_left_after_excluding_ambiguous(processed_cause_coding, cause_type)
  
n_response_left_after_excluding_ambiguous(processed_outcome_coding, outcome_type)
```
...
n_response_after_separation(processed_mistake_type_coding, mistake_type)

n_response_after_separation(processed_cause_coding, cause_type)
  
n_response_after_separation(processed_outcome_coding, outcome_type)
```
...
n_response_left_excluded_during_coding(processed_mistake_type_coding, mistake_type)

n_response_left_excluded_during_coding(processed_cause_coding, cause_type)

n_response_left_excluded_during_coding(processed_outcome_coding, outcome_type)
```
...
n_response_left_excluded_during_grouping(processed_mistake_type_grouped, mistake_type)

n_response_left_excluded_during_grouping(processed_cause_grouped, cause_type)

n_response_left_excluded_during_grouping(processed_outcome_grouped, outcome_type)
```
...
#### Number of responses after excluding anonymous respondents

```{r}
n_responses_after_excluding_anonymous <- function(df, type_var) {
  df %>% 
  ungroup() %>% 
  group_by({{type_var}}) %>% 
  summarise(n = n()) 
...
n_responses_after_excluding_anonymous(processed_mistake_type_filtered, mistake_type)

n_responses_after_excluding_anonymous(processed_cause_filtered, cause_type)
  
n_responses_after_excluding_anonymous(processed_outcome_filtered, outcome_type)
```
...
If the participants response got separated both separated responses got the same code and got dropped.
...
exclude <-  
  ...
  # In two cases the response got separated into 2 responses but both got the same code. In order to keep the numbers constant we keep only one response.
  distinct(response_id, type, .keep_all = TRUE) %>% 
