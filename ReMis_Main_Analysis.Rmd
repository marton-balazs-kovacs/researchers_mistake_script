...
```{r}
descriptives %>% 
  summarise(median_year = median(years, na.rm = TRUE))
```
...
```{r}
ggsave("Figures/figs2.png", plot = last_plot(), dpi = 300, width = 50, height = 30, units = "cm")

ggsave("Figures/figs2.pdf", plot = last_plot(), dpi = 300, width = 50, height = 30, units = "cm")
```
...
descriptives %>% 
  ...
  count() %>% 
  ggplot(.) +
  aes(x = reorder(field, n), y = n) +
  geom_bar(stat = "identity") +
  scale_y_continuous(expand = c(0,0), limits = c(0, 120)) +
  labs(x = "Research Field of the Respondent",
       y = "Count") + 
  coord_flip() +
  theme(
...
```{r}
ggsave("Figures/figs3.png", plot = last_plot(), dpi = 300, width = 50, height = 30, units = "cm")

ggsave("Figures/figs3.pdf", plot = last_plot(), dpi = 300, width = 50, height = 30, units = "cm")
```
...
```{r}
# With percentage
general_mistake_frequency_plot_data <- 
  general_mistake_frequency %>% 
  filter(!is.na(general_frequency)) %>% 
  group_by(general_frequency) %>% 
  summarise(n = n()) %>% 
...
general_mistake_frequency_all_n <- sum(general_mistake_frequency_plot_data$n)
...
general_mistake_frequency_plot <- 
  ...
  labs(x = "Frequency Level",
       title = paste("Overall Frequency (n =", general_mistake_frequency_all_n, ")")) +
  coord_flip() +
  theme(axis.ticks = element_line(color = "black"),
        axis.line = element_line(color = "black"),
...
# With count
general_mistake_frequency_plot_count <- 
  general_mistake_frequency_plot_data %>% 
  ggplot() +
  aes(x = general_frequency, y = n) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = n), size = 8, hjust = -0.3) +
  scale_y_continuous(expand = c(0,0)) +
  labs(x = "Frequency Level",
       title = paste("Overall Frequency (n =", general_mistake_frequency_all_n, ")")) +
  coord_flip() +
...
```{r} 
# With percentage
most_frequent_frequency_plot_data <- 
  general_mistake_overview %>%
  filter(type == "recurring",
         question == "frequency",
         !is.na(rating)) %>% 
...
most_frequent_frequency_all_n <- sum(most_frequent_frequency_plot_data$n)
...
most_frequent_frequency_plot <- 
  ...
  geom_bar(stat = "identity") +
  geom_text(aes(label = paste0(percentage, "%")), size = 8, hjust=-0.3) +
  labs(title = paste("Most Frequent Mistakes (n =", most_frequent_frequency_all_n, ")")) +
  scale_y_continuous(expand = c(0,0), limit = c(0, 75), breaks = c(0, 20, 40, 60, 75), labels = scales::label_percent(scale = 1)) +
  coord_flip() +
  theme(axis.ticks = element_line(color = "black"),
        axis.line = element_line(color = "black"),
...
# With counts
most_frequent_frequency_plot_count <- 
  most_frequent_frequency_plot_data %>% 
  ggplot() +
  aes(x = rating, y = n) +
  geom_bar(stat = "identity") +
...
```{r}
# With percentage
most_frequent_seriousness_plot_data <- 
  general_mistake_overview %>% 
  filter(type == "recurring",
         question == "serious",
         !is.na(rating)) %>% 
...
most_frequent_seriousness_all_n <- sum(most_frequent_seriousness_plot_data$n)
...
most_frequent_seriousness_plot <- 
  ...
  scale_y_continuous(expand = c(0,0), limit = c(0, 75), breaks = c(0, 20, 40, 60, 75), labels = scales::label_percent(scale = 1)) +
  labs(y = "Percentage",
       title = paste("Most Frequent Mistakes (n =", most_frequent_seriousness_all_n, ")")) +
  coord_flip() +
  theme(axis.ticks = element_line(color = "black"),
        axis.line = element_line(color = "black"),
        axis.text = element_text(color = "black", size = 18),
...
# With count
most_frequent_seriousness_plot_count <- 
  most_frequent_seriousness_plot_data %>% 
    ggplot() +
  aes(rating, n) +
  geom_bar(stat = "identity") +
...
```{r}
# With percentage
most_serious_seriousness_plot_data <- 
  general_mistake_overview %>% 
  filter(type == "serious",
         question == "serious",
         !is.na(rating)) %>% 
...
most_serious_seriousness_all_n <- sum(most_serious_seriousness_plot_data$n)
...
most_serious_seriousness_plot <- 
  ...
  labs(x = "Seriousness Level",
       y = "Percentage",
       title = paste("Most Serious Mistakes (n =", most_serious_seriousness_all_n, ")")) +
  coord_flip() +
  theme(axis.ticks = element_line(color = "black"),
        axis.line = element_line(color = "black"),
        axis.text = element_text(color = "black", size = 18),
...
# With count
most_serious_seriousness_plot_count <- 
  most_serious_seriousness_plot_data %>% 
    ggplot() +
  aes(rating, n) +
  geom_bar(stat = "identity") +
...
```{r}
ggsave("Figures/fig3.png", width = 14.4, height = 9, plot = last_plot())

ggsave("Figures/fig3.pdf", width = 14.4, height = 9, plot = last_plot())
```

```{r}
cowplot::plot_grid(general_mistake_frequency_plot_count, most_frequent_frequency_plot_count,
                   most_serious_seriousness_plot_count, most_frequent_seriousness_plot_count, ncol = 2, align = "hv")
```

```{r}
ggsave("Figures/figs4.png", width = 14.4, height = 9, plot = last_plot())

ggsave("Figures/figs4.pdf", width = 14.4, height = 9, plot = last_plot())
```
...
```{r, fig.show = 'hide'}
relationship_plot_data %>% 
    ggplot(aes(x = reorder(label, n_sum), y = prop, group = group_cause_meta, fill = group_cause_meta)) +
    geom_bar(stat = "identity") +
...
ggsave("Figures/cause_type_relationship_plot.pdf", width = 14.4, height = 9, plot = last_plot())
```
...
```{r}
ggsave("Figures/fig4.png", width = 14.4, height = 9, plot = last_plot())

ggsave("Figures/fig4.pdf", width = 14.4, height = 9, plot = last_plot())
```
...
```{r}
ggsave("Figures/figs6.png", width = 14.4, height = 9, plot = last_plot())

ggsave("Figures/figs6.pdf", width = 14.4, height = 9, plot = last_plot())
```
...
Calculating the proportions of outcomes in the sample.
...
Counting how many respondents listed a specific outcome for each mistake type
...
```{r, fig.show = 'hide'}
relationship_plot_data %>% 
    ggplot(aes(x = reorder(label, n_sum), y = prop, group = group_outcome, fill = group_outcome)) +
    geom_bar(stat = "identity") +
    viridis::scale_fill_viridis(discrete = TRUE) +
    scale_y_continuous(expand = c(0,0), limits = c(0, 1), breaks = c(0, 0.25, 0.5, 0.75, 1), labels = scales::percent(c(0, 0.25, 0.5, 0.75, 1))) +
    coord_flip() +
...
ggsave("Figures/outcome_type_relationship_plot.pdf", width = 14.4, height = 9, plot = last_plot())
```
...
```{r}
ggsave("Figures/fig5.png", width = 14.4, height = 9, plot = last_plot())

ggsave("Figures/fig5.pdf", width = 14.4, height = 9, plot = last_plot())
```
...
```{r}
ggsave("Figures/figs5.png", width = 14.4, height = 9, plot = last_plot())

ggsave("Figures/figs5.pdf", width = 14.4, height = 9, plot = last_plot())
```
...
### Counting the number of grouped responses per mistake type
...
```{r}
mistake_type_frequent <- 
  mistake_type_groups %>% 
  filter(type == "recurring") %>% 
  count(group_type) %>% 
  rename(`Most Frequent Mistakes` = n)
```

### Counting how many times a mistake type is reported for the most serious mistakes

```{r}
mistake_type_serious <- 
  mistake_type_groups %>% 
  filter(type == "serious") %>% 
  count(group_type) %>% 
  rename(`Most Serious Mistakes` = n)
```

## Merging tables of most frequent and most serious mistakes

```{r}
mistake_type_table <- 
  mistake_type_frequent %>% 
  left_join(., mistake_type_serious, by ="group_type") %>% 
  rename(`Mistake Type Group` = group_type) %>% 
  mutate(n_all = `Most Frequent Mistakes` + `Most Serious Mistakes`) %>% 
  arrange(desc(n_all)) %>% 
  select(-n_all)
```
...
## Merging tables for the causes of the most frequent and most serious mistakes

```{r}
cause_group_table <- 
  cause_group_frequent %>% 
  left_join(., cause_group_serious, by ="group_cause") %>% 
  rename(`Cause Group` = group_cause) %>% 
  mutate(n_all = `Most Frequent Mistakes` + `Most Serious Mistakes`) %>% 
  arrange(desc(n_all)) %>% 
  select(-n_all)
```
...
outcome_group_table <- 
  outcome_group_frequent %>% 
  left_join(., outcome_group_serious, by ="group_outcome") %>% 
  rename(`Outcome Group` = group_outcome) %>% 
  mutate(n_all = `Most Frequent Mistakes` + `Most Serious Mistakes`) %>% 
  arrange(desc(n_all)) %>% 
  select(-n_all)
```
