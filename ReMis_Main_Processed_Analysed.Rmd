...
```{r}
cause_type_relationship <- cause_type_relationship %>% 
...
relationship_plot_label <- relationship_plot_data %>% 
  ...
  mutate(label = paste(group_cause_meta, prop, "%"),
...
type_analysis %>%
  ...
  #geom_text(aes(label = group_type), hjust = -0.2, vjust = 0.6) +
  geom_label_repel(aes(label = group_type),
                   box.padding   = 0.35, 
                   point.padding = 0.5,
                   segment.color = 'grey50') +
  scale_x_continuous(limits = c(1, 3), breaks = c(1, 2, 3), label = c("Very low", "Low", "Moderate")) +
  scale_y_continuous(limits = c(1, 3), breaks = c(1, 2, 3), label = c("Insignificant", "Minor", "Moderate")) +
